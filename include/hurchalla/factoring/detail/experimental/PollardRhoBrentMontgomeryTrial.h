...
struct PollardRhoBrentMontgomeryTrial {
  ...
  T operator()(const M& mf, T& expected_iterations, C c) const
  {
    ...
    // given that this PollardRhoBrentMontgomeryTrial has not performed as
    // well as other trials, for now we're not taking the time to optimize with
    // expected_iterations.  See PollardRhoBrentTrial for an example of using
    // it to improve perf (very slightly)
