...
struct PollardRhoBrentMontgomeryTrialParallel {
  ...
  T operator()(const M& mf, C c) const
  {
    ...
    constexpr T one_third_gcd_threshold =
                                       HURCHALLA_PRBMTP_ONE_THIRD_GCD_THRESHOLD;
    ...
    for (T i = 0; i < PRE_CYCLE_SIZE; ++i) {
        ...
        bz = mf.fusedSquareSub(bz, negative_c);
    }
...
//        C nega4z = mf.getCanonicalValue(mf.multiply(a1z, mf.add(negt2z, nega3z)));
        FV nega4z = mf.getFusingValue(mf.multiply(a1z, mf.add(negt2z, nega3z)));
...
#  pragma GCC diagnostic pop
            ...
            for (j = 0; j < gcd_loop_size; ++j) {
                ...
                V b2z = mf.fusedSquareSub(bz, negative_c);
                ...
                bz = mf.fusedSquareSub(b2z, negative_c);
                bz = mf.fusedSquareSub(bz, negative_c);
            }
